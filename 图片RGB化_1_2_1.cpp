...
void myRGB(Mat& dst, vector<Point> vec, int a, int b) {
...
int main() {
	...
	Mat src, dst_src, dst, element = getStructuringElement(MORPH_RECT, Size(3, 3));
	...
	if (filePath.substr(filePath.length() - 3, 3) == "ncc") {
		...
		system("mkdir final");
		clearr(dst, ROIArea);
		myRGB(dst, ROIArea, 2, 1);
		myRGB(dst, ROIArea, 1, 0);
		myRGB(dst, ROIArea, 0, 2);
